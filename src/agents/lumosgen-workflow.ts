...
import { ProjectWatcherAgent, ContentAnalyzerAgent, ContentGeneratorAgent } from './lumosgen-agents';

// 创建LumosGen营销自动化工作流
export async function createMarketingWorkflow(apiKey: string) {
  // 创建工作流实例
  const workflow = createLumosGenWorkflow(apiKey);
