/**
 * Enhanced Context Engineering Usage Examples
 * 
 * 演示如何使用新的增强上下文工程系统
 */

import { ContextEngine } from '../src/analysis/ContextEngineering';
import { EnhancedLumosGenWorkflow } from '../src/agents/EnhancedWorkflow';
import { ContextSelector } from '../src/analysis/ContextSelector';
import * as vscode from 'vscode';

// 模拟VS Code输出通道
const mockOutputChannel = {
    appendLine: (message: string) => console.log(`[LumosGen] ${message}`),
    append: (message: string) => console.log(message),
    clear: () => {},
    dispose: () => {},
    hide: () => {},
    show: () => {},
    name: 'LumosGen Enhanced'
} as vscode.OutputChannel;

/**
 * 示例1: 基础上下文分析
 */
export async function basicContextAnalysis() {
    ...
    try {
        ...
        console.log('\n💡 内容机会:');
        result.recommendations.contentOpportunities.forEach(opp => 
            console.log(`  - ${opp}`)
        );
        
        console.log('\n🔧 改进建议:');
        result.recommendations.improvementSuggestions.forEach(suggestion => 
 ...
 * 示例2: 不同策略对比
 */
export async function strategyComparison() {
    console.log('\n🔍 示例2: 不同策略对比');
    
    const engine = new ContextEngine(process.cwd(), mockOutputChannel);
    const strategies: ('minimal' | 'balanced' | 'comprehensive')[] = ['minimal', 'balanced', 'comprehensive'];
 ...
 * 示例3: 特定任务的上下文选择
 */
export async function taskSpecificContext() {
    console.log('\n🔍 示例3: 特定任务的上下文选择');
 ...
 * 示例4: 完整的内容生成工作流
 */
export async function fullContentWorkflow() {
    console.log('\n🔍 示例4: 完整的内容生成工作流');
    ...
    try {
        ...
        const result = await engine.executeFullWorkflow(
            process.cwd(),
            'marketing-content',
            {
                changedFiles: [],
 ...
 * 示例5: 营销准备度评估
 */
export async function marketingReadinessAssessment() {
    console.log('\n🔍 示例5: 营销准备度评估');
 ...
 * 示例6: 增量更新演示
 */
export async function incrementalUpdateDemo() {
    console.log('\n🔍 示例6: 增量更新演示');
    
    const workflow = new EnhancedLumosGenWorkflow(
        process.cwd(),
        mockOutputChannel,
        undefined,
        {
    ...
    try {
        // 初始分析
        console.log('📊 执行初始分析...');
        const initialResult = await workflow.executeEnhancedWorkflow(
            process.cwd(),
            'marketing-content'
        );
        ...
        // 增量更新
        const updateResult = await workflow.updateWithChanges(
            changedFiles,
            initialResult.projectAnalysis
        );
